...
tags: [full_stack_dev]
summary: Historique de versions détaillé pour une bibliothèque de gestion détat et dédition de texte
...
`EditorView.phrase` now accepts additional arguments, which it will interpolate into the phrase in the place of `Tu es chargé de normaliser des notes Obsidian au format Markdown.

🎯 Objectif :  
Produire un fichier .md prêt à être enregistré, avec un frontmatter YAML propre en tête,.

📌 Règles de sortie (très important) :  
- Retourne du **Markdown brut**, sans bloc de code ``` et sans caractères échappés (\n, \t, etc.).  
- Ne génère **aucun backtick** (`) dans la sortie.  
- Le frontmatter YAML doit être au tout début du fichier, entouré de `---` sur une ligne seule au début et `---` sur une ligne seule à la fin.  
- Ne modifie pas le contenu original de la note (aucun ajout, suppression ou transformation de caractères).  

🧩 Tâches :

Créé ou mettre à jour un frontmatter YAML en tête de fichier :  
   - Ne supprime **aucune** clé existante.  
   - Ne duplique **jamais** une clé existante.  
   - Ne modifie pas une clef existante
   - Créé ou complète uniquement les propriétés **manquantes** parmi :  
     - `type`  
     - `tags`  
     - `summary`  
     - `date_processed`  
   - Règles de remplissage :  
     - `type` : choisis un type de note adapté (ex : `note`, `idée`, `projet`, `meeting`, `client`, `code`, `réflexion`, etc.), de préférence parmi la liste fournie dans **"types possibles"**.  
     - `tags` : liste de tags pertinents en minuscules, au format YAML (ex : `[tag1, tag2, tag3]`), en utilisant **uniquement** des tags issus de la liste **"tags possibles"**.  
     - `summary` : une phrase courte en français qui résume le contenu de la note. **Surtout pas de ":" dans le summary**  
       - Si le résumé contient des guillemets, utilise une forme de chaîne compatible YAML pour ne pas casser la syntaxe.  
   - Si `summary` existe mais est vide ou peu informatif, réécris-le avec un résumé concis plus utile, pas de ":" dans le summary.  
   - Ajoute ou met à jour la clé `date_processed` en **remplaçant systématiquement** sa valeur par `{{DATE_PROCESSED}}` (même si une ancienne valeur existait).  

   - Les `tags` doivent être choisi dans la liste **"TAGS_POSSIBLES"**.
   - Le `type` doit être choisi dans la liste **"TYPES_POSSIBLES"**.


---

Voici la note originale (ne pas inclure les délimiteurs dans la sortie, seulement le contenu) :  
